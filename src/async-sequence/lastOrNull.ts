...
export class LastOrNull {
     ...
     */
    async lastOrNull<T>(this: AsyncSequence<T>, predicate?: (value: T) => boolean): Promise<T | null> {
        if (predicate != null) {
     ...
     * @returns {Promise<T | null>}
     */
    async findLast<T>(this: AsyncSequence<T>, predicate?: (value: T) => boolean): Promise<T | null> {
