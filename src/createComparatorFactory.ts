...
function compareBy<T>(keyOrSelector: any): Comparator<T> {
  const selector = asSelector<T>(keyOrSelector);
  return compare<T>((a: T, b: T) => naturalCompare(selector(a), selector(b)));
}

function compareByDescending<T>(keyOrSelector: any): Comparator<T> {
  const selector = asSelector<T>(keyOrSelector);
  return compare<T>((a: T, b: T) => naturalCompare(selector(b), selector(a)));
}

function asSelector<T>(
  keyOrSelector: (item: T) => any | string
): (item: T) => any {
...
function nullsLast<T>(): Comparator<T> {
  return compare<T>((a: T, b: T) => (a === null ? 1 : b === null ? -1 : 0));
}
